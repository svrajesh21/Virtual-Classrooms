...
const TeacherDashboard = () => {
    ...
    useEffect(() => {
        const fetchTeacherDetails = async () => {
            ...
            try {
                const res = await axios.get(`http://localhost:5000/api/teacher/${teacherEmail}`);
                setTeacher(res.data);
                setEditedTeacher(res.data);
            } catch (error) {
    ...
    const handleConfirmClick = async () => {
        try {
            await axios.put(`http://localhost:5000/api/teacher/update`, editedTeacher);
            setTeacher(editedTeacher);
            setIsEditing(false);
        } catch (error) {
            console.error("⚠️ Error updating teacher details", error);
        }
    };
            ...
            <div className={`sidebar ${sidebarOpen ? "open" : ""}`}>
                ...
                {teacher ? (
                    <div className="teacher-info">
                        {isEditing ? (
                            <>
                                <p><strong> Name:</strong> <input type="text" name="name" value={editedTeacher.name} onChange={handleChange} /></p>
                                <p><strong> Email:</strong> <input type="text" name="email" value={editedTeacher.email} onChange={handleChange} disabled /></p>
                                <p><strong> Date of Birth:</strong> <input type="date" name="dob" value={editedTeacher.dob} onChange={handleChange} /></p>
